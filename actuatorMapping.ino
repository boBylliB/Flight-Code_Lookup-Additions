...
void driveActuators(double length1, double length2, double length3) {
  ...
  */ 
  ...
  if (length1 >= lengthTransient) {
    ...
    actuator1.driveTime = abs((length1 - actuator1.currentLength))*accelTime/lengthTransient*1000; // ms
  } else {
    // length1 = currentLength1 + lengthTransient + maxSpeed*(t-accelTime); // m
    actuator1.driveTime = (abs((length1 - actuator1.currentLength - lengthTransient))/maxSpeed + accelTime)*1000; // ms
  }
  if (length2 >= lengthTransient) {
    //length1 = currentLength1 + lengthTransient*t/accelTime; // m
    actuator2.driveTime = abs((length2 - actuator2.currentLength))*accelTime/lengthTransient*1000; // ms
  } else {
    // length1 = currentLength1 + lengthTransient + maxSpeed*(t-accelTime); // m
    actuator2.driveTime = (abs((length2 - actuator2.currentLength - lengthTransient))/maxSpeed + accelTime)*1000; // ms
  }
  if (length3 >= lengthTransient) {
    //length1 = currentLength1 + lengthTransient*t/accelTime; // m
    actuator3.driveTime = abs((length3 - actuator3.currentLength))*accelTime/lengthTransient*1000; // s
  } else {
    // length1 = currentLength1 + lengthTransient + maxSpeed*(t-accelTime); // m
    actuator3.driveTime = (abs((length3 - actuator3.currentLength - lengthTransient))/maxSpeed + accelTime)*1000; // s
  }
